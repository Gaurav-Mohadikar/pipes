...
function Attendance() {
  ...
  const toggleAttendance = async (employeeId, date) => {
    ...
    try {
      console.log(`Sending request to update attendance for ${employeeId} on ${dateStr} to ${newStatus}`);
  
      const response = await fetch(`${API_BASE_URL}/attendance/${employeeId}`, { // Fixed URL
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        const text = await response.text(); // Read error response
      ...
      setEmployees((prevEmployees) =>
        prevEmployees.map((emp) =>
          emp._id === employeeId
            ? { ...emp, attendance: { ...emp.attendance, [dateStr]: newStatus } }
            : emp
        )
      );
